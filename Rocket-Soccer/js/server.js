...
function connected(socket) {
    ...
    socket.on('clientName', (data, response) => {
        ...
        let newPlayer = new Player_S(STADIUM_W / 2 + PAD_LENGTH / 2, 270 - yPadDiff / 2, STADIUM_W / 2 - PAD_LENGTH / 2, 270 - yPadDiff / 2, PAD_WIDTH, 0, PAD_MASS);
        ...
        if (clientNoInRoom == NB_PLAYERS_IN_GAME) {
            ...
            // ball
            if (!football_S.has(room)) {
                let newBall = new Ball_S(STADIUM_W / 2, 270, BALL_RADIUS, BALL_MASS);
                newBall.layer = room;
...
function newRandomBall() {
    ...
    switch (ballType) {
        ...
        case BALL_TYPE_S.CAPSULE:
            ball = new Capsule_S(STADIUM_W / 2 - BALL_CAPSULE_LENGTH / 2, 270, STADIUM_W / 2 + BALL_CAPSULE_LENGTH / 2, 270, BALL_RADIUS, BALL_RADIUS, BALL_MASS);
            ball.pos.set(STADIUM_W / 2, 270);
            ball.vel.set(0, 0);
            return ball;
    }
