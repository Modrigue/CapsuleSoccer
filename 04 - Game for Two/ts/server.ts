...
let playerPos: Map<string, {x: number, y: number, no: number, angle: number}>
    = new Map<string, {x: number, y: number, no: number, angle: number}>();
let serverBalls: Map<string, Player_S> = new Map<string, Player_S>();
let football_S: Ball_S;
...
function connected(socket: any)
{
    ...
    if (playerPos.size == 0)
    {
        ...
        playerPos.set(socket.id, {x: 115, y: 270, no: 1, angle: 0});
    }
    else if (playerPos.size == 1)
    {
        ...
        newPlayer.no = 2;
        serverBalls.set(socket.id, newPlayer);
        playerPos.set(socket.id, {x: 525, y: 270, no: 2, angle: 0});

        // create football
        football_S = new Ball_S(320, 270, 20, 6);
        io.emit('updateFootball', {x: football_S.pos.x, y: football_S.pos.y});
    }
    else if (playerPos.size >= 2)
    {
        console.log("too many players, bye");
        return false;
    }
    
    const json: string = JSON.stringify(Array.from(playerPos)); // only works with 1D map
    io.emit('updateConnections', json);
    ...
    socket.on('disconnect', function()
    {
        ...
        console.log("Current number of players: " + playerPos.size);
...
function serverLoop()
{
    ...
    {
        let playerPosCur: {x: number, y: number, angle: number, no: number}
            = <{x: number, y: number, angle: number, no: number}>playerPos.get(id);

        playerPosCur.x = player.pos.x;
        playerPosCur.y = player.pos.y;
        playerPosCur.angle = player.angle;
        playerPosCur.no = player.no;
    }

    if (football_S === undefined)
    {
    ...
    {
        gameLogic_S();
        footballPos = {x: football_S.pos.x, y: football_S.pos.y};
    }
...
function gameLogic_S()
{
    if(football_S.pos.x < 45 || football_S.pos.x > 595){
        scoring();
    }
    for(let [id, player] of serverBalls)
    {
...
function gameSetup()
{
    ...
    football_S.pos.set(320, 270);
    football_S.vel.set(0, 0);
}
