...
let clientBalls: Map<string, Capsule> = new Map<string, Capsule>();
let selfID;
...
socket.on('connect', () => {
    ...
    (<Capsule>clientBalls.get(socket.id)).player = true;
    (<Capsule>clientBalls.get(socket.id)).maxSpeed = 5;
    (<Capsule>clientBalls.get(socket.id)).angle = 0;
