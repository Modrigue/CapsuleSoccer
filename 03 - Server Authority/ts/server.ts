...
let playerPos: Map<string, {x: number, y: number, angle: number}>
    = new Map<string, {x: number, y: number, angle: number}>();
let serverBalls: Map<string, Capsule_S> = new Map<string, Capsule_S>();
...
function connected(socket: any)
{
    socket.on('newPlayer', (playerCoords: any) => {
        ...
        (<Capsule_S>serverBalls.get(socket.id)).maxSpeed = 5;
        (<Capsule_S>serverBalls.get(socket.id)).angle = 0;
    ...
    socket.on('disconnect', function()
    {
        ...
        {
            (<Capsule_S>serverBalls.get(socket.id)).remove();
...
function serverLoop_S()
{
    ...
    {
        let playerPosCur: {x: number, y: number, angle: number}
            = <{x: number, y: number, angle: number}>playerPos.get(id);
